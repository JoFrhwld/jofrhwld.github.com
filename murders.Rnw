...
\SweaveOpts{cache=T, fig.path='figures/murder-', error=F, warning=F,message=F}
...
\frametitle{Two Philadelphias}
<<plot2, echo = F, eval=F>>=
	ggplot(philly.bw.count.w, aes(month.date, 1/(freq/ndays), color = race)) + 
		geom_point()+
		stat_smooth(method = gam, formula = y ~ s(x, bs = "cs"))+
		scale_y_continuous(breaks = c(0.5,1,2,7,14,21,31), trans = revlog_trans(base = 2))+
		expand_limits(y = 0.5)+
		...
		xlab("Date (by month)")+
		theme_bw()+
		facet_wrap(~weapon, scales = "free_y")+
		scale_color_brewer(palette = "Set1")
@
