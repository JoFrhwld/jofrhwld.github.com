...
anae.plot <- function(input, output){
	...
	if(a[length(a)]=="csv"){
		anae <<- read.csv(input)	
	}else if((a[length(a)]=="xls")){
		anae <<- read.xls(input)	
	}else{
		anae <<- read.delim(input)		
	}
...
file.path(directory, "ANAE")->location
#paste(directory, "ANAE", sep = "/")->location
dir.create(location)
#system(paste("mkdir", location, sep = " "))
setwd(location)

if(strsplit(sessionInfo()$R.version$platform,"-")[[1]][2]=="apple"){
x11()
dev.off()	
}
...
for(i in levels(data$DIALECT)){
	...
	drop.levels(reg)->reg
	file.path(location,i)->jawn1
	file.path(jawn1, "SPCOM")->jawn2
